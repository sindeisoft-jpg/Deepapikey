...
class FixedDeepSeekBrowser(QMainWindow):
    """修复版主窗口类，解决网页交互问题"""
    ...
    def init_ui(self):
        """初始化用户界面"""
        self.setWindowTitle("DeepSeek Qt浏览器 - 修复版")
    ...
    def create_browser_panel(self):
        ...
        self.debug_button.setToolTip("启用开发者工具和调试模式")
        
        # 设置按钮样式
        for btn in [self.back_button, self.forward_button, self.refresh_button, 
                   self.home_button, self.go_button, self.debug_button]:
        ...
        # 设置默认URL
        self.browser.setUrl(QUrl("https://www.deepseek.com"))
    ...
    def create_chat_panel(self):
        ...
        # 网页调试信息
        debug_group = QGroupBox("网页调试信息")
        debug_layout = QVBoxLayout()
        
        self.debug_info = QTextEdit()
        self.debug_info.setReadOnly(True)
        self.debug_info.setMaximumHeight(100)
        self.debug_info.setPlaceholderText("JavaScript控制台消息将显示在这里...")
    ...
    def setup_connections(self):
        ...
        self.debug_button.clicked.connect(self.toggle_debug_tools)
    ...
    def on_load_started(self):
        ...
        self.debug_info.append(f"[{datetime.now().strftime('%H:%M:%S')}] 开始加载页面...")
    ...
    def on_load_finished(self, success):
        ...
        if success:
            ...
            self.debug_info.append(f"[{datetime.now().strftime('%H:%M:%S')}] 页面加载完成")
            
            # 页面加载完成后，注入一些辅助JavaScript
            self.inject_helper_scripts()
        else:
            self.status_label.setText("页面加载失败")
            self.debug_info.append(f"[{datetime.now().strftime('%H:%M:%S')}] 页面加载失败")
    ...
    def on_js_console_message(self, level, message, line_number, source_id):
        ...
        log_msg = f"[{datetime.now().strftime('%H:%M:%S')}] JS {level_str}: {message}"
        if line_number:
            log_msg += f" (line {line_number})"
            
        self.debug_info.append(log_msg)
        
        # 自动滚动到底部
        scrollbar = self.debug_info.verticalScrollBar()
        scrollbar.setValue(scrollbar.maximum())
        
    def inject_helper_scripts(self):
        """注入辅助JavaScript脚本"""
        # 注入一些帮助网页交互的JavaScript
        helper_script = """
        // 辅助脚本：修复可能的交互问题
        console.log('DeepSeek Qt浏览器辅助脚本已注入');
        ...
        document.addEventListener('click', function(e) {
            ...
            if (target && target.tagName === 'A') {
                console.log('链接点击:', target.href);
            }
        });
        """
        
        self.browser.page().runJavaScript(helper_script)
    ...
    def toggle_debug_tools(self):
        ...
        if hasattr(page, 'devToolsPage') and page.devToolsPage():
            ...
            self.debug_button.setText("调试")
        else:
            # 打开开发者工具
            page.setDevToolsPage(page)
            self.debug_button.setText("关闭调试")
    ...
    def export_conversation(self):
        ...
        # 这里可以添加文件保存对话框
        # 暂时先显示在消息框中
        history_text = "DeepSeek对话历史\n" + "="*50 + "\n\n"
        
        for i, entry in enumerate(self.conversation_history, 1):
            history_text += f"对话 {i} [{entry['timestamp']}]\n"
            history_text += f"风格: {entry['style']}\n"
            history_text += f"您: {entry['user']}\n"
            history_text += f"DeepSeek: {entry['ai']}\n"
            history_text += "-"*50 + "\n\n"
...
def main():
    ...
    app.setApplicationName("DeepSeek Qt浏览器 - 修复版")
